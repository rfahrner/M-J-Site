...
create table if not exists public.paperwork_submission_events (
  ...
  created_by uuid references auth.users(id) on delete set null,
  created_at timestamptz not null default now(),
  constraint paperwork_submission_events_type_check check (event_type in ('submitted','auto_attached','archived_matched','needs_review','reattached','note_added','deleted','restored')),
